...
// Definició propietats de reflexió (emissió, ambient, difusa, especular) del material.
	...
	//Vector donde indicaremos la posicion relativa de las piezas (TEMPORAL)
	//(La idea es que este vector se pase por parametro, y ademas, que haya un vector por tipo de pieza, 
	//		ya que todas las piezas del mismo tipo se dibujan a la vez)

	std::vector<vec3> position;

	switch (objecte)
	{
	case CHESSBOARD:		//Caso donde queremos mostrar el tablero (de momento solo varios peones simultaneos)
		

		//For que inicializa posiciones para mostrar diferentes peones
		...
		for (int i = 0; i < 10; i++)
		{
			for (int j = 0; j < 10; j++)
			{
				position.push_back(glm::vec3(i * 50.0f - (10 - i) * 50.0f, j * 50.0f - (10 - j) * 50.0f, 0.0f));
			}
		}
		Pawn(sh_programID, MatriuVista, MatriuTG, sw_mat, position);

		/*
		for (CPieza peon : tableroGlobal->m_piezas)
		{
			Pawn(sh_programID, MatriuVista, MatriuTG, sw_mat, peon.m_posicion);
		}
		*/

		break;
	
	case PAWN:			//Dibuja un solo peon

		//Posición relativa a la que se colocara el peon
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Funcion que dibuja la pieza de ajedrez "Peon" (Pawn) de forma relativa a la posicion pasada por paramtro.
// 
//PARAMETROS ANADIDOS:
// position: vector de vectores de 3 dimensiones. El numero de elementos del vector representa el numero de peones que 
// van a dibujarse (si solo hay 1 posicion, solo se dibuja 1 peon). En cada posicion se indica la posicion relativa donde
// se dibujara cada peon.
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void Pawn(GLint sh_programID, glm::mat4 MatriuVista, glm::mat4 MatriuTG, bool sw_mat[5], std::vector<vec3> position)
{
	glm::mat4 NormalMatrix(1.0), ModelMatrix(1.0);

	
	////Tapa inferior de la base
	//Set_VAOList(BASE1_PIEZA, loadgluDisk_EBO(0.0, 45.0, 10, 1));		// Genera VAO del disco
	for (int i = 0; i < position.size(); i++)
	{
		ModelMatrix = glm::translate(MatriuTG, position[i] + vec3(0.0f, 0.0f, 0.0f));
		...
		//gluDisk(0.0, 45.0, 10, 1); //Disco inferior
		draw_TriEBO_Object(BASE1_PIEZA);
	}
	////Fin tapa inferior de la base


	////Cilindro1 base
	//Set_VAOList(GLUT_CYLINDER, loadgluCylinder_EBO(45.0f, 50.0f, 7.5f, 10, 1));		// Genera EBO del cilindro
	for (int i = 0; i < position.size(); i++)
	{
		ModelMatrix = glm::translate(MatriuTG, position[i] + vec3(0.0f, 0.0f, 0.0f));
		ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 1.0f, 1.0f));
		...
		//Cilindre amb radi base (1), radi tapa (2), alzada (3), nombre de cares (4) y nombre de cilindres apilats (5)
		draw_TriEBO_Object(BASE2_PIEZA);
	}
	////FIN Cilindro1 base

	////Cilindro2 base
	//Set_VAOList(BASE3_PIEZA, loadgluCylinder_EBO(50.0f, 50.0f, 8.0f, 10, 1));		// Genera EBO del cilindro
	for (int i = 0; i < position.size(); i++)
	{
		ModelMatrix = glm::translate(MatriuTG, position[i] + vec3(0.0f, 0.0f, 7.5f));
		ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 1.0f, 1.0f));
		// Pas ModelView Matrix a shader
		glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
		// Pas NormalMatrix a shader
		...
		//Cilindre amb radi base (1), radi tapa (2), alzada (3), nombre de cares (4) y nombre de cilindres apilats (5)
		draw_TriEBO_Object(BASE3_PIEZA);
	}
	////FIN Cilindro2 base


	////Cilindro3 base
	//Set_VAOList(BASE4_PIEZA, loadgluCylinder_EBO(50.0f, 30.0f, 10.0f, 10, 1));		// Genera EBO del cilindro
	for (int i = 0; i < position.size(); i++)
	{
		ModelMatrix = glm::translate(MatriuTG, position[i] + vec3(0.0f, 0.0f, 15.5f));
		ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 1.0f, 1.0f));
		// Pas ModelView Matrix a shader
		glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
		// Pas NormalMatrix a shader
		...
		//Cilindre amb radi base (1), radi tapa (2), alzada (3), nombre de cares (4) y nombre de cilindres apilats (5)
		draw_TriEBO_Object(BASE4_PIEZA);
	}
	////FIN Cilindro3 base
	...
	//INICIO TRONCO
	////Cilindro1 tronco
	//Set_VAOList(GLUT_CYLINDER, loadgluCylinder_EBO(30.0f, 25.0f, 15.0f, 10, 1));		// Genera EBO del cilindro
	for (int i = 0; i < position.size(); i++)
	{
		ModelMatrix = glm::translate(MatriuTG, position[i] + vec3(0.0f, 0.0f, 25.5f));
		ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 1.0f, 1.0f));
		// Pas ModelView Matrix a shader
		glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
		// Pas NormalMatrix a shader
		...
		//Cilindre amb radi base (1), radi tapa (2), alzada (3), nombre de cares (4) y nombre de cilindres apilats (5)
		draw_TriEBO_Object(PEON_TRONCO1);
	}
	////FIN Cilindro1 tronco


	////ANILLO CENTRAL
	//Set_VAOList(GLU_CILINDRE_SENCER, loadCilindre_VAO(32.5f, 27.5f, 15.0f, 10, 1));		// Genera VAO del cilindro
	for (int i = 0; i < position.size(); i++)
	{
		ModelMatrix = glm::translate(MatriuTG, position[i] + vec3(0.0f, 0.0f, 40.5f));
		ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 1.0f, 1.0f));
		// Pas ModelView Matrix a shader
		glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
		// Pas NormalMatrix a shader
		...
		//Cilindre amb radi base (1), radi tapa (2), alzada (3), nombre de cares (4) y nombre de cilindres apilats (5)
		//cilindre(32.5f, 27.5f, 15.0f, 10, 1);		// Genera cilindre sencer
		draw_TriVAO_Object(PEON_ANILLO);
	}
	////FIN ANILLO CENTRAL

	////Cilindro2 tronco
	//Set_VAOList(GLUT_CYLINDER, loadgluCylinder_EBO(20.0f, 14.16f, 17.5f, 10, 1));		// Genera EBO del cilindro
	for (int i = 0; i < position.size(); i++)
	{
		ModelMatrix = glm::translate(MatriuTG, position[i] + vec3(0.0f, 0.0f, 55.0f));
		ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 1.0f, 1.0f));
		// Pas ModelView Matrix a shader
		glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
		// Pas NormalMatrix a shader
		...
		//Cilindre amb radi base (1), radi tapa (2), alzada (3), nombre de cares (4) y nombre de cilindres apilats (5)
		draw_TriEBO_Object(PEON_TRONCO2); //Dibuja el cilindro cargado al 
	}
	////FIN Cilindro2 tronco

	////Cilindro3 tronco
	//Set_VAOList(GLUT_CYLINDER, loadgluCylinder_EBO(14.16f, 14.16f, 4.0f, 10, 1));		// Genera EBO del cilindro
	for (int i = 0; i < position.size(); i++)
	{
		ModelMatrix = glm::translate(MatriuTG, position[i] + vec3(0.0f, 0.0f, 72.5f));
		ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 1.0f, 1.0f));
		// Pas ModelView Matrix a shader
		glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
		// Pas NormalMatrix a shader
		...
		glUniformMatrix4fv(glGetUniformLocation(sh_programID, "normalMatrix"), 1, GL_FALSE, &NormalMatrix[0][0]);
		//Cilindre amb radi base (1), radi tapa (2), alzada (3), nombre de cares (4) y nombre de cilindres apilats (5)
		draw_TriEBO_Object(PEON_TRONCO3);
	}
	////FIN Cilindro3 tronco

	//Collar superior 
	//Set_VAOList(GLU_CILINDRE_SENCER, loadCilindre_VAO(20.0f, 20.0f, 6.0f, 10, 1));		// Genera VAO del cilindro
	for (int i = 0; i < position.size(); i++)
	{
		ModelMatrix = glm::translate(MatriuTG, position[i] + vec3(0.0f, 0.0f, 76.5f));
		ModelMatrix = glm::scale(ModelMatrix, vec3(1.0f, 1.0f, 1.0f));
		// Pas ModelView Matrix a shader
		glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
		// Pas NormalMatrix a shader
		...
		//Cilindre amb radi base (1), radi tapa (2), alzada (3), nombre de cares (4) y nombre de cilindres apilats (5)
		//cilindre(20.0f, 20.0f, 6.0f, 10, 1); //Cilindre SENCER (Amb tapes superior i inferior)
		draw_TriVAO_Object(GLU_CILINDRE_SENCER);
	}
	//FIN Collar superior
	...
	//Esfera superior
	//Set_VAOList(GLU_SPHERE, loadgluSphere_EBO(18.0, 20, 20));	// Genera EBO de la esfera 
	
	//glPushMatrix();
			//glTranslated(0.0, 5.0, 6.5);
	for (int i = 0; i < position.size(); i++)
	{
		ModelMatrix = glm::translate(MatriuTG, position[i] + vec3(0.0f, 0.0f, 92.5f));
		// Pas ModelView Matrix a shader
		glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
		// Pas NormalMatrix a shader
		NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
